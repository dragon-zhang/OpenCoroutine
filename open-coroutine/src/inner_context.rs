...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct InnerContext(pub(crate) &'static c_void);
...
impl InnerContext {
    ...
    #[inline(always)]
    pub(crate) fn new(stack: &Stack, f: ContextFn) -> InnerContext {
        InnerContext(unsafe { make_fcontext(stack.top(), stack.len(), f) })
    }
    ...
    #[inline(always)]
    pub(crate) fn switch(to: &InnerContext, param: *mut c_void) -> Transfer {
...
impl Transfer {
    ...
    #[inline(always)]
    pub fn new(context: InnerContext, data: *mut c_void) -> Transfer {
        Transfer {
            context,
            data,
        }
    ...
    pub fn switch(to: &Transfer) -> Transfer {
        InnerContext::switch(&to.context, to.data)
    }
}
...
mod tests {
    ...
    fn test() {
        ...
        let context = InnerContext::new(&stack, context_function);
